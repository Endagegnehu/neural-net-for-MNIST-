...
# Random Initialize Weights
intial_theta1 = neuralNet.randInitializeWeights(
    input_layer_size, hidden_layer_size)
intial_theta2 = neuralNet.randInitializeWeights(hidden_layer_size, num_labels)

initial_nn_params = np.append(
    intial_theta1.reshape(10025), intial_theta2.reshape(260))

x0 = intial_theta1
lambda_ = 0.1
myargs = (X, y_matrix,intial_theta2,lambda_)

theta = optmz.fmin_bfgs(neuralNet.cost_grad, x0,
                        fprime=neuralNet.grad_func,
                        args=myargs,
                        epsilon=lambda_,
                        maxiter=500
                        )
